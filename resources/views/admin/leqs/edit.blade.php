...
<div class="card border-primary">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.leq.title_singular') }}
    </div>

    <div class="card-body text-info">
        <form method="POST" action="{{ route("admin.leqs.update", [$leq->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-row">
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('layene_siyasi') ? 'is-invalid' : '' }}" name="layene_siyasi_id" id="layene_siyasi_id" required>
                    @foreach($layene_siyasis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('layene_siyasi_id') ? old('layene_siyasi_id') : $leq->layene_siyasi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $leq->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('leq_code') ? 'is-invalid' : '' }}" type="text" name="leq_code" id="leq_code" value="{{ old('leq_code', $leq->leq_code) }}" required>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('jimara_dengderan') ? 'is-invalid' : '' }}" type="number" name="jimara_dengderan" id="jimara_dengderan" value="{{ old('jimara_dengderan', $leq->jimara_dengderan) }}" step="1">
