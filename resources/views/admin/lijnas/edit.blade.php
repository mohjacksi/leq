...
<div class="card border-primary">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.lijna.title_singular') }}
    </div>

    <div class="card-body text-info">
        <form method="POST" action="{{ route("admin.lijnas.update", [$lijna->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-row">
            <div class="form-group col-md-6">
                <label class="required" for="name">{{ trans('cruds.lijna.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $lijna->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('lijna_code') ? 'is-invalid' : '' }}" type="text" name="lijna_code" id="lijna_code" value="{{ old('lijna_code', $lijna->lijna_code) }}" required>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('leq') ? 'is-invalid' : '' }}" name="leq_id" id="leq_id">
                    @foreach($leqs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('leq_id') ? old('leq_id') : $lijna->leq->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('leq'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('jimara_dengderan') ? 'is-invalid' : '' }}" type="number" name="jimara_dengderan" id="jimara_dengderan" value="{{ old('jimara_dengderan', $lijna->jimara_dengderan) }}" step="1">
