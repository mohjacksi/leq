...
<div class="card border-primary">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.kandid.title_singular') }}
    </div>

    <div class="card-body text-info">
        <form method="POST" action="{{ route("admin.kandids.update", [$kandid->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-row">
            <div class="form-group col-md-6">
                <label class="required" for="nav">{{ trans('cruds.kandid.fields.nav') }}</label>
                <input class="form-control {{ $errors->has('nav') ? 'is-invalid' : '' }}" type="text" name="nav" id="nav" value="{{ old('nav', $kandid->nav) }}" required>
                @if($errors->has('nav'))
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('jimara_kandidi') ? 'is-invalid' : '' }}" type="text" name="jimara_kandidi" id="jimara_kandidi" value="{{ old('jimara_kandidi', $kandid->jimara_kandidi) }}" required>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('layene_siyasi') ? 'is-invalid' : '' }}" name="layene_siyasi_id" id="layene_siyasi_id">
                    @foreach($layene_siyasis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('layene_siyasi_id') ? old('layene_siyasi_id') : $kandid->layene_siyasi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('extra') ? 'is-invalid' : '' }}" type="text" name="extra" id="extra" value="{{ old('extra', $kandid->extra) }}">
                @if($errors->has('extra'))
                    <div class="invalid-feedback">
                        {{ $errors->first('extra') }}
                    </div>
