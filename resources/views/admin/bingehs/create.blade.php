...
<div class="card border-primary">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.bingeh.title_singular') }}
    </div>

    <div class="card-body text-info">
        <form method="POST" action="{{ route("admin.bingehs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-row">
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('bingeh_code') ? 'is-invalid' : '' }}" type="text" name="bingeh_code" id="bingeh_code" value="{{ old('bingeh_code', '') }}" required>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('rekxraw') ? 'is-invalid' : '' }}" name="rekxraw_id" id="rekxraw_id">
                    @foreach($rekxraws as $id => $entry)
                        <option value="{{ $id }}" {{ old('rekxraw_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('jimara_rekxistiya') ? 'is-invalid' : '' }}" type="text" name="jimara_rekxistiya" id="jimara_rekxistiya" value="{{ old('jimara_rekxistiya', '') }}">
