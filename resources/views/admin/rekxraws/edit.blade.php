...
<div class="card border-primary">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.rekxraw.title_singular') }}
    </div>

    <div class="card-body text-info">
        <form method="POST" action="{{ route("admin.rekxraws.update", [$rekxraw->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-row">
            <div class="form-group col-md-6">
                <label class="required" for="name">{{ trans('cruds.rekxraw.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $rekxraw->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('code_rekxraw') ? 'is-invalid' : '' }}" type="text" name="code_rekxraw" id="code_rekxraw" value="{{ old('code_rekxraw', $rekxraw->code_rekxraw) }}" required>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('lijna') ? 'is-invalid' : '' }}" name="lijna_id" id="lijna_id" required>
                    @foreach($lijnas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lijna_id') ? old('lijna_id') : $rekxraw->lijna->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('lijna'))
                    <div class="invalid-feedback">
