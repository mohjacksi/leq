...
<div class="card border-primary">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.daxlkrnaDengenKandida.title_singular') }}
    </div>

    <div class="card-body text-info">
        <form method="POST" action="{{ route("admin.daxlkrna-dengen-kandidas.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-row">
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('layenesiyasi') ? 'is-invalid' : '' }}" name="layenesiyasi_id" id="layenesiyasi_id" required>
                    @foreach($layenesiyasis as $id => $entry)
                        <option value="{{ $id }}" {{ old('layenesiyasi_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('jimara_kandidi') ? 'is-invalid' : '' }}" name="jimara_kandidi_id" id="jimara_kandidi_id" required>
                    @foreach($jimara_kandidis as $id => $entry)
                        <option value="{{ $id }}" {{ old('jimara_kandidi_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
