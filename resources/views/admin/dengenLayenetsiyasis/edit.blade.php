...
<div class="card border-primary">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.dengenLayenetsiyasi.title_singular') }}
    </div>

    <div class="card-body text-info">
        <form method="POST" action="{{ route("admin.dengen-layenetsiyasis.update", [$dengenLayenetsiyasi->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-row">
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('leq') ? 'is-invalid' : '' }}" name="leq_id" id="leq_id" required>
                    @foreach($leqs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('leq_id') ? old('leq_id') : $dengenLayenetsiyasi->leq->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('leq'))
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('lijna') ? 'is-invalid' : '' }}" name="lijna_id" id="lijna_id" required>
                    @foreach($lijnas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lijna_id') ? old('lijna_id') : $dengenLayenetsiyasi->lijna->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('lijna'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('bingeh') ? 'is-invalid' : '' }}" name="bingeh_id" id="bingeh_id" required>
                    @foreach($bingehs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bingeh_id') ? old('bingeh_id') : $dengenLayenetsiyasi->bingeh->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('westgeh') ? 'is-invalid' : '' }}" name="westgeh_id" id="westgeh_id" required>
                    @foreach($westgehs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('westgeh_id') ? old('westgeh_id') : $dengenLayenetsiyasi->westgeh->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('layene_siyasi') ? 'is-invalid' : '' }}" name="layene_siyasi_id" id="layene_siyasi_id" required>
                    @foreach($layene_siyasis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('layene_siyasi_id') ? old('layene_siyasi_id') : $dengenLayenetsiyasi->layene_siyasi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('jimara_dengan') ? 'is-invalid' : '' }}" type="number" name="jimara_dengan" id="jimara_dengan" value="{{ old('jimara_dengan', $dengenLayenetsiyasi->jimara_dengan) }}" step="1" required>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('extra_1') ? 'is-invalid' : '' }}" type="text" name="extra_1" id="extra_1" value="{{ old('extra_1', $dengenLayenetsiyasi->extra_1) }}">
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('extra_2') ? 'is-invalid' : '' }}" type="text" name="extra_2" id="extra_2" value="{{ old('extra_2', $dengenLayenetsiyasi->extra_2) }}">
